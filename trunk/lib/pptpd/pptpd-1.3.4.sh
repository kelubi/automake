...
SRC_URI="http://www.ctohome.com/FuWuQi/8b/291.html"
PKG_NAME='pptpd'
N="pptpd"
V="1.3.4"

INST_HOME="/opt/install/$N-$V"
if [ ! -d $INST_HOME ]; then
	mkdir -pv $INST_HOME
fi 
cd $INST_HOME
...
rm -rvf /etc/pptpd.conf
rm -rvf /etc/ppp

wget http://lnmpp.googlecode.com/files/dkms-2.0.17.5-1.noarch.rpm
wget http://lnmpp.googlecode.com/files/kernel_ppp_mppe-1.0.2-3dkms.noarch.rpm
wget http://lnmpp.googlecode.com/files/pptpd-1.3.4-1.rhel5.1.i386.rpm
wget http://lnmpp.googlecode.com/files/ppp-2.4.4-9.0.rhel5.i386.rpm
