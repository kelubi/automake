...
if [ -s $PKG_NAME ]; then
  ...
  wget -c $SRC_URI
  wget -c http://mirrors.dev.shopex.cn/lnmpp/sources/php-5.2.14-fpm-0.5.14.diff.gz
fi
...
gzip -cd php-5.2.14-fpm-0.5.14.diff.gz | patch -d $N-$V$R -p1
cd $N-$V$R
