...
SRC_URI="http://lnmpp.googlecode.com/files/nginx-0.8.41.tar.gz"
...
cat > $F_DEST/conf/ecos_admin.conf<<'EOF'
if ($request_uri ~ (.+?\.php)(|/.+)$ ){
        break;
}
...
server
{
        ...
        #include deny_list.conf;
        
        #admin panel setting
        location ~ /(index.php/|)shopadmin {
            #include deny_admin.conf;
            #include error_pages.conf;
            include ecos_admin.conf;
